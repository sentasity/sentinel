...
The enqueue is also the skip cache: one investigation per issue, per environment, per
release. Sentry sends repeats on purpose, since a frequency rule re-fires for as long
as an issue stays hot, and each repeat posts a card of its own. Investigating that card
would read the same code and reach the same findings, so a repeat on an unchanged release
answers its own thread with a pointer to the thread that has them. The one exception is
an earlier attempt that failed: the next repeat takes the row over and runs the
investigation again under the new card, once. A new release gets a fresh row, because the
code changed and the findings may too.
...
second one afterwards. The receiver answers the findings POST with one grant per approved
finding: a dispatch id and a one-time callback token, with the repository, the base
branch, and the callback URL alongside. No credential is in that response.
...
test that fails without it, and sends the changed files back to the receiver as whole
text, with the base commit and the pull request title and body. The receiver checks
every path against the same policy the gate uses, mints a token scoped to one repository
and two permissions, builds one commit on a new branch through GitHub's API, opens the
pull request as the App, and posts the link into the Teams thread. A grant that never
reports is failed into the thread once its hour is up.

The gate stays on the receiver rather than moving into the session, and so does the
only GitHub credential in the pipeline. [The security model](/sentinel/security-model/)
