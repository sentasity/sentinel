...
2. Update the key everywhere it lives, one place at a time. Set `AWS_REGION` to the
   region you deployed into first; `scripts/put-parameters.sh` and the receiver both
   default to `us-east-1`, but nothing requires it.
