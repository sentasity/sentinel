...
- **The disclosure scan reads and nothing else.** `runner/disclosure_runner.py` gives its session `Read`, `Glob`, and `Grep`, no `Bash`, no write tools, and `setting_sources=[]`, because the tree it reviews is also where an injected `CLAUDE.md`, skill, or hook would be waiting. Its result block is authoritative only when it carries a token generated after that tree was already fixed, so a findings block planted in a file cannot claim the run's verdict. That token also delimits the untrusted blocks in the prompt, which is why the template is filled in a single `re.sub` pass: the content under review routinely contains the template's own markers, and substituting them in sequence would stamp the live token into the untrusted region and let that content forge a delimiter. The runner prints only a fixed set of finding fields, so a value smuggled into an extra field never reaches the public log. Findings and gaps are in [OPERATIONS.md](OPERATIONS.md).
...
- One accepted residual risk: the Agent SDK forwards the process environment to its subprocess, so the workflow's OAuth token is technically reachable from inside either session. Containment for that is the runner's egress posture, not the runner code. The disclosure session is the narrower of the two, holding no shell and no fetch tool, but "narrower" is not "none".
